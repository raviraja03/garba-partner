...
|---|---|---|---|
| **0** | Foundation | Monorepo scaffolding, config/shared packages, lint/format/test tooling, CI, API skeleton (health check, error handler, response envelope, logging with redaction), DB connection + migration runner, web/admin shells with routing and Tailwind | All root scripts pass. `GET /api/v1/health` works. A migration runs up and down |
| **1** | Member auth & profile | OTP (provider + dev test numbers), sessions, onboarding, 18+ gate, profile, photo upload/EXIF stripping, cities/areas seed, settings, account deletion (deactivation part) | Member can sign up, onboard, edit profile, log out, delete |
